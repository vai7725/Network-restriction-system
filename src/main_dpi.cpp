...
Examples:
              ...
              << R"( capture.pcap filtered.pcap
  )" << program
              << R"( capture.pcap filtered.pcap --block-app YouTube
  )" << program
              << R"( capture.pcap filtered.pcap --block-ip 192.168.1.50 --block-domain *.tiktok.com
  )" << program
              << R"( capture.pcap filtered.pcap --rules blocking_rules.txt
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "Failed to initialize NRS engine\n";
        return 1;
    }

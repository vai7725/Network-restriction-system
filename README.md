# NRS Engine - Network Restriction System
...
**Network Restriction System (NRS)** is a technology used to examine the contents of network packets as they pass through a checkpoint. Unlike simple firewalls that only look at packet headers (source/destination IP), NRS looks _inside_ the packet payload.
...
### What Our NRS Engine Does:

```
User Traffic (PCAP) → [NRS Engine] → Filtered Traffic (PCAP)
                           ↓
...
**This is the key to NRS**: Even though HTTPS is encrypted, the domain name is visible in the first packet!
...
| Multi-threaded           | `src/nrs_mt.cpp`       | Production, large captures |
...
│   └── nrs_engine.h           # Main orchestrator
│
...
│   ├── nrs_mt.cpp             # ★ MULTI-THREADED VERSION ★
...
### Step 5: Extract SNI (Network Restriction System)
...
The multi-threaded version (`nrs_mt.cpp`) adds **parallelism** for high performance:
...
2. **Fast Paths (FPs):** Do the actual NRS processing
...
```bash
g++ -std=c++17 -O2 -I include -o nrs_simple \
...
```bash
g++ -std=c++17 -pthread -O2 -I include -o nrs_engine \
    src/nrs_mt.cpp \
    src/pcap_reader.cpp \
    src/packet_parser.cpp \
    src/sni_extractor.cpp \
...
```bash
./nrs_engine test_dpi.pcap output.pcap
```
...
```bash
./nrs_engine input.pcap output.pcap --lbs 4 --fps 4
...
```
╔══════════════════════════════════════════════════════════════╗
║              NRS ENGINE v2.0 (Multi-threaded)                 ║
╠══════════════════════════════════════════════════════════════╣
...
This NRS engine demonstrates:

1. **Network Protocol Parsing** - Understanding packet structure
2. **Network Restriction System** - Looking inside encrypted connections
